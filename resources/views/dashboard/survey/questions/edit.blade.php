            ...
            <div class="row">
                ...
                <div class="col-md-2">
                    ...
                    <select name="question_has_other" id="question_has_other" class="form-control">
                        <option value="0" {{ $question->has_other == 1 ?: 'selected'}}>No</option>
                        <option value="1" {{ $question->has_other == 0 ?: 'selected'}}>Yes</option>
                    </select>
                </div>
            </div>
            <hr>
...
function cerateAnswer(value) {
    $.ajax({
        method: 'POST',
        url: "{{route('dashboard.ajax.createAnswer')}}",
        data: {
            _token: "{{ csrf_token() }}",
            body: value,
...
function cerateMultipleAnswers(answers) {
    $.ajax({
        method: 'POST',
        url: "{{ route('dashboard.ajax.cerateMultipleAnswers') }}",
        data: {
            _token: "{{ csrf_token() }}",
            answers: answers,
            question_id: "{{ $question->id }}"
...
function saveEditedAnswer(btn, id) {
    ...
    $.ajax({
        method: 'POST',
        url: "{{route('dashboard.ajax.editAnswer', ':id')}}".replace(':id', id),
        data: {
            _token: "{{ csrf_token() }}",
            body: value,
            question_id: "{{ $question->id }}"
...
const removeAnswer = (a, id) => {
    $.ajax({
        method: 'POST',
        url: "{{route('dashboard.ajax.deleteAnswer', ':id')}}".replace(':id', id),
        data: {
            _token: "{{ csrf_token() }}"
        },
        success: (res) => {
